...
#define MOTOR (*((volatile uint32_t *)(0x42000000 + (0x400073FC-0x40000000)*32 + 0*4)))
...
//volume part
    uint32_t getVolume()
    {
        ...
        TIMER1_TAMR_R =0x0;//edit for later
    ...
    float x= ((y-39999671.45)/(-113.4545455));
        //return x;
        float v= 3.976*x;
        //return v;
    ...
    bool isWateringAllowed(int second1, int second2){
        bool WAX=false;
...
int main(void)
{
    ...
    for (i = 0; i < data.fieldCount; i++)
     ...
     putsUart0("\r");
            ...
            if((lightlevel>20)&&(volumelevel<150)){
                playWaterLowAlert();
                waitMicrosecond(5000000);
            }
            if((lightlevel>20)&&(batterylevel<1)){
                playBatteryLowAlert();
                waitMicrosecond(5000000);
            }
